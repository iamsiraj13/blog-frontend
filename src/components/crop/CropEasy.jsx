...
const CropEasy = ({ photo, setOpenCrop }) => {
  ...
  const [cropedAreaPexels, setCropAreaPexels] = useState(null);
  ...
  const handleCropComplet = (cropArea, cropedAreaPexels) => {
    setCropAreaPexels(cropedAreaPexels);
  };

  const handleCropImage = async () => {
    try {
      const croppedImg = await getCroppedImg(photo?.url, cropedAreaPexels);
  ...
  return (
    <div className="fixed z-100 top-[100px] inset-0 bg-black/50 flex justify-center p-5 overflow-auto">
      <div className="bg-white h-fit w-full sm:max-w-[350px] rounded-lg p-5">
